...
int main(int argc,char** argv){
  ...
  double Qr[2][2]={{0.006,0},{0,0.006}};
  double Rr[2][2]={{0.0001,0},{0,0.0001}};
  ...
  double Qv[2][2]={{0.3,0},{0,0.3}};
  double Rv[2][2]={{1,0},{0,0.03}};
...
//******************* Translational KF ********************//
	...
	// Motion model //
    for(int i=0;i<2;i++){
      v_est_temp[i] = v_est_last[i] + dt * a[i];
    }
...
//   y = y + dx * sinf(phi) + dy * cosf(phi);
    ...
    odom_quat = tf::createQuaternionMsgFromYaw(phi_est[0]);
/*
...
//    odom.pose.pose.orientation.w = q[3];
	...
	ROS_INFO("Pr : %lf %lf %lf %lf",Pr[0][0],Pr[0][1],Pr[1][0],Pr[1][1]);

	ROS_INFO("Pv : %lf %lf %lf %lf",Pv[0][0],Pv[0][1],Pv[1][0],Pv[1][1]);
